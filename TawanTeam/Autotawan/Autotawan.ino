...
long prevtime = 0;
long prevtime2 = 0;
int speedPWM[7]={0,0,0,0,1,120,0};//0-3 pwm,4rail,5updown,6lock
//int vsetpoint = 4000;
bool state =0;
...
PIDControl PIDControllers[] = {
  {1.0, 0.005, 2.0, 0, 0,0},// PID for Front Left Motor 
...
//   if(Serial.available()>0){ // input serial
//     for (int i = 0 ; i < 4; i++) {
...
//   }
  ...
  if (millis() - prevtime >= 100) {
    ...
    int speedBR = PIDMOTOR(speedPWM[3],Count[3].Vrpm,100, PIDControllers[3]);
     ...
     for(int i = 0 ;i<4 ;i++){
       Serial.print(speedPWM[i]);
       Serial.print(" ");
       Serial.print(Count[i].Vrpm);
       Serial.print(" ");
       
    }
    Serial.print(Count[0].Vrpm);
    Serial.println("");
  ...
  RailServo(speedPWM[4]);
  UpDownServo(speedPWM[5]);
  Lock(speedPWM[6]);
}
